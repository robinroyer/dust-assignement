...
CLI usage:
  python dust_client.py list-data-sources <space_id>
  python dust_client.py list-documents <space_id> <ds_id> [--limit N] [--offset N]
  python dust_client.py get-document <space_id> <ds_id> <document_id>
  python dust_client.py upsert-document <space_id> <ds_id> <document_id> --text TEXT [--title TITLE] [--source-url URL] [--tags tag1,tag2] [--mime-type TYPE] [--timestamp TS] [--light] [--async]
  python dust_client.py delete-document <space_id> <ds_id> <document_id>
  python dust_client.py search-documents <space_id> <ds_id> <query> [--top-k N] [--tags tag1,tag2]
"""
...
class DustDataSourcesClient(AbstractDataSource):
    ...
    def from_env(cls) -> "DustDataSourcesClient":
        """Instantiate from DUST_API_KEY and DUST_WORKSPACE_ID env vars."""
    ...
    # ------------------------------------------------------------------
    # Mappers: raw API dicts → dataclasses
    # ------------------------------------------------------------------
    ...
    def _map_section(raw: Dict[str, Any]) -> Section:
        return Section(
            ...
            sections=[
                DustDataSourcesClient._map_section(s) for s in raw.get("sections", [])
            ],
        )
    ...
    def _serialize_section(section: Section) -> Dict[str, Any]:
        ...
        if section.sections:
            d["sections"] = [
                DustDataSourcesClient._serialize_section(s) for s in section.sections
            ]
        return d
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="dust_client",
        description="CLI for the Dust DataSources API",
    )
    sub = parser.add_subparsers(dest="command", required=True)

    # list-data-sources
    p = sub.add_parser("list-data-sources", help="List data sources in a space")
    p.add_argument("space_id")

    # list-documents
    p = sub.add_parser("list-documents", help="List documents in a data source")
    p.add_argument("space_id")
    ...
    # get-document
    p = sub.add_parser("get-document", help="Get a single document")
    p.add_argument("space_id")
    p.add_argument("ds_id")
    p.add_argument("document_id")

    # upsert-document
    p = sub.add_parser("upsert-document", help="Create or update a document")
    p.add_argument("space_id")
    p.add_argument("ds_id")
    p.add_argument("document_id")
    p.add_argument("--text", default=None, help="Document text content")
    p.add_argument("--title", default=None)
    p.add_argument("--source-url", default=None, dest="source_url")
    p.add_argument("--mime-type", default=None, dest="mime_type")
    p.add_argument("--tags", default=None, help="Comma-separated list of tags")
    p.add_argument("--timestamp", type=int, default=None, help="Unix timestamp in milliseconds")
    p.add_argument("--light", action="store_true", help="Light document output (no vectors/chunks)")
    p.add_argument("--async", action="store_true", dest="async_processing", help="Asynchronous processing")

    # delete-document
    p = sub.add_parser("delete-document", help="Delete a document")
    p.add_argument("space_id")
    p.add_argument("ds_id")
    p.add_argument("document_id")

    # search-documents
    p = sub.add_parser("search-documents", help="Search documents in a data source")
    p.add_argument("space_id")
    p.add_argument("ds_id")
    p.add_argument("query")
    p.add_argument("--top-k", type=int, default=10, dest="top_k")
    p.add_argument("--tags", default=None, help="Comma-separated list of tags to filter by")
    ...
    try:
        if args.command == "list-data-sources":
            ...
            _print_json(client.upsert_document(
                ...
                light_document_output=args.light,
